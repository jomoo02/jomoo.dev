<template>
    <div class="relative grid grid-cols-10 gap-6 md:gap-8">
        ...
        <div class="min-w-full col-span-10 md:col-span-8 pb-8" >
            ...
            <div class="flex flex-col gap-2.5 md:flex-row w-full" :class="pageNumber < postStore.vocaPosts.length-1 ? 'justify-between':'justify-end'">
                <div v-if="pageNumber < postStore.vocaPosts.length-1" class="md:w-1/3">
                    <PostMoveCard :postDirection="BEFORE" :pageNumber="pageNumber+1" dataKind="voca"></PostMoveCard>
                </div>
                <div v-if="pageNumber > 0" class="md:w-1/3">
                    <PostMoveCard :postDirection="AFTER" :pageNumber="pageNumber-1" dataKind="voca"></PostMoveCard>
                </div>
            </div>
        </div>
    </div>
