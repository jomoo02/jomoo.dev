...
1. h2 태그와 h3 태그로 목차의 섹션을 알린다.
2. h3 태그는 h2 태그보다 오른쪽 위치 시켜 서브 섹션임을 알린다.  
...
## 구현하려는 기능 1번, 2번 구현
구현하려는 기능 **1. h2 태그와 h2 태그로 목차의 섹션을 알린다**와 **2. h3 태그는 h2 태그보다 오른쪽 위치 시켜 서브 섹션임을 알린다**를 한 번에 구현하겠다.  
...
배열 `props.links`를 `v-for`로 순회해 h2 태그의 text들을 화면에 렌더링하며 순회하는 h2 섹션에 children(h3)이 존재하는 경우 h3 섹션들을 순회해 렌더링한다.   
...
### 페이지 `pages/[category]/[detail]/[post].vue` 코드 추가
``` vue [pages/\\[category]/\\[detail]/\\[post].vue] meta-info=val
<template>
  <!--  -->
...
하지만 현재 목차의 항목을 클릭하면 목차가 헤더에 가려져 보이지 않는다. 이를 해결하기 위해 `scroll-margin-top`을 이용한다.  
...
`app.config.js`에 `prose-h2:scroll-mt-[6.3rem]`와 `prose-h3:scroll-mt-[6.4rem]`을 추가한다.  
위 클래스값들을 `[category]/[detail]/[post].vue`의 post에 적용하면 된다.
...
이로써 목차의 항목들을 클릭해 해당 섹션으로 이동하면 헤더에 가려지지 않고 클릭한 목차가 화면에 보이도록 이동한다.  

### 추가로, 부드러운 scroll
현재 목차를 클릭하여 섹션으로 이동 시 한 번에 이동한다. 이보다는 애니메이션처럼 부드럽게 이동하는 것이 자연스럽다고 생각해 추가하려 한다.  
`nuxt.config`에 router options에 원하는 스크롤 타입을 추가하면 된다. 
```js 
...
**현재 화면의 위치에 따라 목차의 섹션을 스타일링한다**는 현재 화면에 보이는 섹션의 목차 항목을 스타일링해 지금 어떤 섹션을 보고 있는지 알리는 기능이다.  

### 절대 좌표를 이용해 가장 가까운 섹션을 스타일링
처음에 떠오른 방식으로, 렌더링 된 h2, h3 태그들을 가지고 와 태그들의 절대 좌표를 구한 뒤, 스크롤 이벤트가 발생하면 현재 스크롤 위치가 어떤 태그와 가장 가까운지 구해 가장 가까운 태그를 스타일링 하는 것이었다. 하지만 문제가 있었다. 
...
`setTimeout`을 이용해 문제를 해결했지만 사진, 영상의 load가 1초보다 늦으면 같은 문제가 발생할 것이다. 사진, 영상의 load가 완료된 시점을 정확하게 구하면 완벽하게 해결되지만 아쉽게도 좋은 방법이 떠오르지 않았다.  
...
중간에 nuxt 버전을 업그레이드했는데 `TypeError: Cannot read properties of null (reading 'parentNode') when using keepAlive` 오류가 발생해 동작이 안 돼 당황했다. vue upstream issue라 하여 버전을 다운시키기도 했었다. (nuxt github issue 주소 [ㄴlink](https://github.com/nuxt/nuxt/issues/25751))
...
목차 기능 구현을 마무리로, jomoo.dev에서 생각했던 목표 기능들을 다 구현했다.  
이후에는 추가하고 싶은 기능이 생기면 구현하고 일단, CS 공부를 하려 한다.
