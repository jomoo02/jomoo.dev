...
```
위 코드에서는 리팩터링 하기 전 코드다. `WordList.vue`에서 `WordCard.vue`에 단어의 정보들을 전달하고(prop), 해당 단어를 클릭하면, 커스텀 이벤트 `@wordDetail`을 `WordList.vue`에서 감지해, pinia 저장소에 해당 단어를 전달해 모달을 여는 방식이었다.  
...
- `props`의 속성 중 `means`만 조작이 필요해 `computed`를 이용했고 나머지 속성들은 조작이 필요 없어 ref로 감싸지 않았다. props는 부모 속성이 업데이트되면 자식으로 흐르기 때문에 그대로 사용한다.  
- 변수 `openWordDetail`와 함수 `toggleDetailOpen`을 이용해 단어를 클릭하면 모달을 화면에 띄우고, 모달을 종료하는 명령이 오면 `toggleDetailOpen`을 호출해 모달을 닫는다.
